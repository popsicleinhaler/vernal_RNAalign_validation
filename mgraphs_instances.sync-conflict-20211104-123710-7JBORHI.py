...
def get_instance_k_motif(k):
	...
	for i in mmaga.nodes[list(mmaga.nodes())[k]]['node_set']:
		...
		n+=1
		# saving the instances of motif k where k is an integer deignating maga graph nodes (0-9403)
		# 2 -motif starts at  160
		# 4 -motif starts at  1892
		# 5 -motif starts at  2629
		# 6 -motif starts at  3496
		# 7 -motif starts at  4004
		# as a list of nodes in the maga graph then using the meta graph to reverse node_map these nodes
		# to their pdbid-chain-residue identity store as dictionary instance: dot separated list
	mmd = {}
	for k,v in motif_dict.items():
		rrs = []
		for n in v:
			r = mmg.reversed_node_map[n]
...
#added this function just to encapsulate the loose code without commenting it if i wanna import the above function
def get_motifs():
